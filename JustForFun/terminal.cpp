...
*/
void my_print(const char * cnt, int bytes){
...
void loadFiles(FILE *fat12, int father, int start, int ents){
    ...
    for (int i = 0; i < ents; i++){ // 循环次数等于根目录区条目数，需要另外计算
        fseek(fat12, start+ENT_SIZE*i, SEEK_SET); // 为什么读取时候都要seek一下？
    ...
    for (int i = l;i<r;i++){
        ...
        // 加载目录的子节点，但是要排除.和..
        // 除此之外，需要考量子目录所占扇区超过一个的情况，这里的实现暂时没有考量
